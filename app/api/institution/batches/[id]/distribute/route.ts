...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  console.log('[distribute] incoming request for batch:', batchId)
  try {
    const body = await request.json()
    console.log('[distribute] body:', body)
    ...
    console.log('[distribute] adminProfile:', adminProfile)
    console.log('[distribute] batchRow:', batchRow, 'batchError:', batchError)

    if (batchError || !batchRow || batchRow.institution_id !== institutionId) {
      console.warn('[distribute] batch validation failed - batchRow or institution mismatch')
