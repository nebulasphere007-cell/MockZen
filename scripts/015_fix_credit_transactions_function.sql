-- Fix distribute_credits_to_batch to insert into `credit_transactions` using `delta` (existing schema)
...
BEGIN
  ...
  -- Use `delta` column in credit_transactions (schema uses `delta`)
  INSERT INTO public.credit_transactions (user_id, delta, reason, metadata)
  SELECT user_id, p_amount_per_member, 'batch_distribution', jsonb_build_object('batch_id', p_batch_id, 'performed_by', p_performed_by) FROM members;
