-- Fix the distribute_credits_to_batch function to avoid using a non-existent 'created_by' column
-- and instead store the performer in the metadata.
...
BEGIN
  ...
  SELECT user_id, p_amount_per_member, 'batch_distribution', jsonb_build_object('batch_id', p_batch_id) FROM members;
